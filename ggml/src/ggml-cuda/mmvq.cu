...
// Parallel projections are submitted consecutively and the pool returns the
// same temporary address. Reuse only under both conditions; logical identity
// prevents allocator address reuse across different activations.
// The entry is additionally stamped with a generation that graph_compute bumps
// per evaluation: tensor arena addresses, graph-allocator offsets AND the pool
// allocation sequence all repeat across evals with different tensor contents,
// so an (identity, data, pool-ptr, dims) match from a previous eval is a false
// hit that serves stale quantized activations. (Observed: EP prefill garbage
// once GGML_CUDA_MOE_F16GATHER shifted the pool layout into such a collision.)
...
// P2 (2026-07-14): batch>1 (ncols_dst 2..8) MMVQ tuning for Pascal SM60. Under MTP
// self-speculation the verify forward runs at ncols_dst = spec_draft_n_max+1 (=5 for
// n-max 4) and is 58.8% of decode GPU time (nsys mtp-attribution-20260714), yet the
// winner's SM60 tuning covered only ncols_dst==1 — batch>1 fell through to the GENERIC
// table (nwarps=2, rows=2) at ~2x the batch-1 per-byte cost. Isolated sweep on the real
// TP shapes found rows_per_block=6 (nwarps=2) optimal for Q8_0 ncols_dst 5..8: vocab
// proj -25.5%, per-forward matvec -14%, end-to-end MTP decode +7.1% lossless
// (results/mmvq-batch-20260714). MID/NWARPS_HI defaults still equal GENERIC; only
// ROWS_HI is retuned (2 -> 6). Override any of these at build time to re-sweep.
#ifndef GGML_CUDA_SM60_MMVQ_NWARPS_MID    // ncols_dst 2..4 (default 4 == generic)
#define GGML_CUDA_SM60_MMVQ_NWARPS_MID 4
#endif
#ifndef GGML_CUDA_SM60_MMVQ_NWARPS_HI     // ncols_dst 5..8 (default 2 == generic)
#define GGML_CUDA_SM60_MMVQ_NWARPS_HI 2
#endif
#ifndef GGML_CUDA_SM60_MMVQ_ROWS_HI       // rows_per_block, Q8_0, ncols_dst 2..8 (P2-tuned: 6)
...
static constexpr __host__ __device__ int calc_nwarps(ggml_type type, int ncols_dst, mmvq_parameter_table_id table_id) {
    if (table_id == MMVQ_PARAMETERS_PASCAL_SM60) {
        ...
        if (ncols_dst >= 2 && ncols_dst <= 4) {
            return GGML_CUDA_SM60_MMVQ_NWARPS_MID;   // default 4 == generic
        }
        if (ncols_dst >= 5 && ncols_dst <= 8) {
            return GGML_CUDA_SM60_MMVQ_NWARPS_HI;     // default 2 == generic
        }
        return 1;
    }
...
static constexpr __host__ __device__ int calc_rows_per_block(
    ...
    if (table_id == MMVQ_PARAMETERS_PASCAL_SM60 && type == GGML_TYPE_Q8_0 && !small_k) {
        ...
        if (ncols_dst >= 2 && ncols_dst <= 8) {
            return GGML_CUDA_SM60_MMVQ_ROWS_HI;   // default 2 == generic
        }
    }
    if (table_id == MMVQ_PARAMETERS_GENERIC || table_id == MMVQ_PARAMETERS_PASCAL_SM60 || table_id == MMVQ_PARAMETERS_GCN || table_id == MMVQ_PARAMETERS_TURING) {
        switch (ncols_dst) {
            case 1:
...
static __global__ void mul_mat_vec_q(
    ...
    // [TAG_MOE_EP] Stage C expert-parallel decode: ids carry GLOBAL expert indices but this
    // device's src0 shard holds only nchannels_x LOCAL experts [base, base+nchannels_x).
    // Non-local (token,slot) channels write ZEROS so the meta backend's PARTIAL all-reduce
    // sums the per-shard outputs into the exact MoE result. ep_base_enc: 0 = no EP, base+1 = EP.
    if (ncols_dst == 1 && ids && ep_base_enc != 0) {
        const int32_t ep_local = (int32_t) channel_x - (int32_t) (ep_base_enc - 1);
        if (ep_local < 0 || ep_local >= (int32_t) nchannels_x) {
            if (threadIdx.y == 0) {
...
#pragma unroll
            ...
            // P5a (upstream 683f0c72e #25445): write back inside the unrolled i-loop
            // and index tmp[j][i] by the compile-time row i (threadIdx.x == i) instead
            // of the runtime tmp[j][threadIdx.x] — keeps the accumulator in registers
            // rather than spilling to local memory. Fusion logic unchanged.
            if (threadIdx.x == i && (rows_per_cuda_block == 1 || uint32_t(row0 + i) < stride_col_dst)) {
...
static __global__ void mul_mat_vec_q_moe(
    ...
    // [TAG_MOE_EP] Stage C expert-parallel decode: ids are GLOBAL, src0 holds nchannels_x LOCAL
    // experts [base, base+nchannels_x). Non-local (token,slot) rows stay 0 (tmp init) so the
    // meta backend's PARTIAL all-reduce sums the per-shard outputs exactly. 0 = no EP.
    bool ep_is_local = true;
    if (ep_base_enc != 0) {
        const int32_t ep_local = (int32_t) channel_x - (int32_t) (ep_base_enc - 1);
        ep_is_local = ep_local >= 0 && ep_local < (int32_t) nchannels_x;
        channel_x   = ep_is_local ? (uint32_t) ep_local : 0;
    }
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride, const uint32_t nchannels_x, const uint32_t ep_base_enc, cudaStream_t stream) {
...
static void mul_mat_vec_q_switch_ncols_dst(
        ...
        const int ids_stride, const uint32_t ep_base_enc, cudaStream_t stream) {
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        mul_mat_vec_q_moe_launch<type>(
            ...
            stride_row_x, stride_col_y, stride_col_dst,
            stride_channel_x, stride_channel_y, stride_channel_dst,
            ncols_dst, ids_stride, (uint32_t) nchannels_x, ep_base_enc, warp_size, nchannels_dst, stream);
        return;
    }
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            if (use_small_k) {
                ...
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, true>(
                    ...
                    stride_sample_x, stride_sample_y, stride_sample_dst, dims.first, dims.second, 0, ids_stride, (uint32_t) nchannels_x, ep_base_enc,
                    stream);
            } else {
                std::pair<dim3, dim3> dims = calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst,
        ...
        case 2: {
            ...
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 dims.first, dims.second, 0, ids_stride, (uint32_t) nchannels_x, ep_base_enc, stream);
        } break;
        case 3: {
        ...
        case GGML_TYPE_Q1_0:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_Q1_0>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, ep_base_enc, stream);
            break;
        case GGML_TYPE_Q4_0:
...
void ggml_cuda_mul_mat_vec_q(
    ...
    bool q8_1_cache_hit = q8_1_dedup_enabled && q8_1_cache.gen == ggml_cuda_mmvq_q8_1_gen &&
    ...
    if (q8_1_dedup_enabled) {
        q8_1_cache.gen = ggml_cuda_mmvq_q8_1_gen;
        q8_1_cache.identity = q8_1_identity;
    ...
    // [TAG_MOE_EP] Stage C: the meta backend stamps op_params[15] = expert_base+1 on each
    // device's MUL_MAT_ID slice when src0 is expert-sharded (0 = no EP). For the fused
    // up+gate+GLU launch, dst is the GLU node — the stamp lives on its MUL_MAT_ID source.
    uint32_t ep_base_enc = 0;
    if (ids) {
        const ggml_tensor * mmid = dst->op == GGML_OP_MUL_MAT_ID ? dst :
            (dst->op == GGML_OP_GLU && dst->src[0] && dst->src[0]->op == GGML_OP_MUL_MAT_ID ? dst->src[0] : nullptr);
        if (mmid) {
            ep_base_enc = (uint32_t) mmid->op_params[GGML_MAX_OP_PARAMS/sizeof(int32_t) - 1];
        }
    }
    ...
    mul_mat_vec_q_switch_type(
        ...
        ne03,              ne3,           s03, s13,              s3,               ids_stride, ep_base_enc, stream);
}

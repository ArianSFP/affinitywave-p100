...
SOURCE="$RESULTS/p100-router-selector-probe.cu"
BINARY="$RESULTS/p100-router-selector-probe"
COORDINATION="$ROOT/bench/COORDINATION-20260725.md"
...
if [ ! -f "$SOURCE" ] || [ ! -x "$NVCC" ]; then
    echo "probe source or CUDA 12.8 nvcc is missing" >&2
    exit 2
fi
if [ -e "$REPORT.jsonl" ] ||
   [ -e "$REPORT.err" ] ||
...
COMMON_ENV=(
    ...
    LD_LIBRARY_PATH="$CUDA_ROOT/lib64:/usr/lib/x86_64-linux-gnu"

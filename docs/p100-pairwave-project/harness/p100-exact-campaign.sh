...
RUN_RESULTS=${RUN_RESULTS:-"$ROOT/results/qwen36-35b-moe-pp-20260721/diagonal-wave-p100-2900"}
CAMPAIGNS_DIR=${CAMPAIGNS_DIR:-"$RUN_RESULTS/p100-exact-campaigns"}
...
artifact_collision() {
    ...
    for candidate in \
        "$RUN_RESULTS/$tag.out" \
        "$RUN_RESULTS/$tag.err" \
        "$RUN_RESULTS/$tag-dumps" \
        "$RUN_RESULTS/$tag-logits-c512.bin" \
        "$RUN_RESULTS/$tag.nsys-rep" \
        "$RUN_RESULTS/$tag.sqlite" \
        "$RUN_RESULTS/$tag.analysis.json" \
        "$RUN_RESULTS/$tag.analysis.md"; do
        if [[ -e "$candidate" ]]; then
            printf '%s\n' "$candidate"
            return 0
        fi
...
verify_artifacts() {
    ...
    off_dump_dir="$RUN_RESULTS/$off_dump_tag-dumps"
    on_dump_dir="$RUN_RESULTS/$on_dump_tag-dumps"
    off_logits="$RUN_RESULTS/$off_logits_tag-logits-c512.bin"
    on_logits="$RUN_RESULTS/$on_logits_tag-logits-c512.bin"
...
run_leg() {
    ...
    if ! env RESULTS="$RUN_RESULTS" \
            "$HARNESS" "$runner_mode" "$tag" "$tokens" 2 f32 "$tokens" "$exact"; then
...
run_campaign() {
    ...
    if ! "$SUMMARY_HELPER" --gate "$campaign_dir" "$RUN_RESULTS" > "$summary"; then
...
case "$MODE" in
    ...
    summary)
        campaign_dir=$(resolve_existing_campaign)
        validate_harness
        "$SUMMARY_HELPER" "$campaign_dir" "$RUN_RESULTS"
        ;;
    *)

...
REFERENCE=${REFERENCE:-"$DOCS_ROOT/evidence/runtime"}
FRONTIER=${FRONTIER:-"$DOCS_ROOT/evidence/frontier"}
MODEL=${MODEL:-/home/arian/models/qwen3.6-35b-a3b/Qwen3.6-35B-A3B-Q8_0.gguf}
CORPUS=${CORPUS:-/home/arian/llama.cpp-q36-decodeopt/wikitext-2-raw/wiki.test.raw}
...
COMMON_ENV=(
    ...
    LD_LIBRARY_PATH="$BUILD/bin:$CUDA_ROOT/lib64:$SYSTEM_LIB"
...
case "$MODE" in
    ...
    bench)
        ...
        env -i "${COMMON_ENV[@]}" "${BASE_ENV[@]}" "${AW_ENV[@]}" "${PAIR_ENV[@]}" "${PAIR_EXACT_ENV[@]}" \
            ...
            taskset --cpu-list "$CPU_LIST" "$BUILD/bin/llama-bench" \
    ...
    diagonal-ppl-save)
        ...
        env -i "${COMMON_ENV[@]}" "${BASE_ENV[@]}" "${AW_ENV[@]}" "${DIAGONAL_ENV[@]}" \
            timeout --signal=TERM --kill-after=10s 900 \
            taskset --cpu-list "$CPU_LIST" "$BUILD/bin/llama-perplexity" \

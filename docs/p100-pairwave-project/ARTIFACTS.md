...
| Q8_0 GGUF (excluded) | `c1283d8b80c3e38b2735ddbc9766d3b3126f44d6c484be419d4e101d09a76131` |
| WikiText-2 `wiki.test.raw` (excluded) | `173c87a53759e0201f33e0ccf978e510c2042d7f2cb78229d9a50d79b9e7dd08` |
| Production EPLB map | `e7ea71d1a6939a49152f8aae3b6792cc1d98a6be31e7abdd411c6e8466a5de85` |
| AffinityWave placement map | `96de3b381bb197b5d843bc9e536496114b74db2cfb77d6f7bf254c75dd851030` |
| Exact campaign summary helper | `fbbd944214dfd08e2324960f0a9cc67a5ba4943eaddddaebc116846e0fd459fa` |
| Nsight mapped-trace analyzer | `42288219ca2d8c3fa3fbef8e5e7cbd40078a23f842370c911f42d6fd52d912f4` |
| Router selector probe source | `6d6acb98e8df1b2218aabf03c0a1ece25152400583e1a8ecf5f10fb4716cd1f3` |
| Stacked projection probe source | `f3c52e86993afd5a7b6275e859bdf8709131e1fc09df88f1e6e16c9d3feb4108` |
| Original `f177771d8` host-streaming source diff over `a7c878670` | `420fe139769a64c8ae7ac91844a52dcc39688e870de4ba17a977d03870b2fbef` |
| Original `f177771d8` host-streaming `libggml-cuda.so` | `e7580770cbfa0cd7c7865295e74f8ebb884a6e51393d0e463905ad3cd3f548ed` |
| Original `f177771d8` host-streaming `libggml.so` | `d28cee388c13ddd7ea4b7eeb1a12208d9c40db61358016f7be8f6ce74e30d080` |
| Original `f177771d8` host-streaming `libllama.so` | `a027464a0a25f1d549003ab158b011bfdede127baac6a994add381f3b9851e02` |
...
The production EPLB and AffinityWave placement maps are committed under
`evidence/runtime`. They are the original model-specific inputs used by the
qualification harness; model weights and corpus data remain excluded.

The original host-streaming source hash covers the `f177771d8`
implementation, tests, and benchmark files. Later residency, owner-return,
and analyzer changes are documented for continuity but are not source
content of this production-diagonal branch and are not covered by that hash.
...
- the campaign gate and summary helper;
- the final static archive script;
- the router selector probe;
- the stacked projection probe.

The copied runners derive repository/evidence paths and accept environment
overrides for machine-specific locations. Archived campaign metadata retains
the original absolute paths for provenance. Inspect the lock, watchdog, and
process-matching behavior before executing a runner on another machine.

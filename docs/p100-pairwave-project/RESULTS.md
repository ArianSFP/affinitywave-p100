...
| Production exact-diagonal commit | `36bad6bb3ad4c9b31edc4dae9fb6c3716b95704b` |
...
"Production-qualified" in this report means exact and repeat-qualified on
the private four-P100 rig. The path remains opt-in and is neither an
upstream llama.cpp default nor a deployment claim.
...
| --- | ---: | --- | --- |
| Exact diagonal N2048 + CohortRail | 2912.264 tok/s | paired pp8128 runtime, complete exactness | rig-qualified, opt-in |
...
| PairFold loader-host down-only 0:39 | 2423.44 tok/s | bypass-off pp8128 exactness and five-sample warm runtime | experimental, default-off |
| PairFold loader-host all projections 0:39 | 1967.42 tok/s | diagnostic pp8128 timing, accepted c512 logits hash | experimental, default-off |
...
- measures 4131.291 ms / 1967.42 tok/s at warm pp8128;
- leaves little host-memory margin on the 46 GiB qualification machine.

That all-host timing predates the pp8128 local-owner-bypass restriction and
is diagnostic rather than long-shape exactness-qualified. The safe
down-only policy disables the bypass, reclaims 1.840 GiB/GPU, matches the
resident pp8128 PPL and logits hash, and measures
3353.908 ms / 2423.44 tok/s.

The model's MTP `blk.40` and non-expert tensors remain resident. The later
PairFold and host-streaming implementations are documented here but are not
part of this branch's production source. Full evidence and details are in

...
Build A (the round-3 "grouped GEMM GO" from the `qwen36-35b-moe-pp-roadmap`
record) is **BUILT and
...
mainloop (`moe-gemm.cu:114-134`), output in expert-sorted order. Continues
[the Qwen MoE execution record](qwen36-35b-moe-pp-execution.md).
...
alternative - only baseline is Build A's own FP32 loop). NOT blocked by the
`p100-fp16-half2-refuted` record

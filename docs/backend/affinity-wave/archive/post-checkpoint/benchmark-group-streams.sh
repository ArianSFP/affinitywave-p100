...
CORRIDOR_ASYNC=${2:-0}
TAG=${3:-group-streams}
GROUP_PATTERN=${4:-1111}
CORRIDOR_EARLY=${5:-0}
WAVE_INSPECT=${6:-0}
CORRIDOR_STATE_SPLIT=${7:-0}
M64_ALL=${8:-0}
...
err="$RESULTS/$TAG.err"
echo "RUN START tag=$TAG p=$P corridor_async=$CORRIDOR_ASYNC group_pattern=$GROUP_PATTERN corridor_early=$CORRIDOR_EARLY wave_inspect=$WAVE_INSPECT corridor_state_split=$CORRIDOR_STATE_SPLIT m64_all=$M64_ALL $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
env -i \
    ...
    GGML_CUDA_AW_M64_ALL="$M64_ALL" \
    GGML_CUDA_AW_HOME=layer \
    GGML_CUDA_AW_DECODE=t64 \
    GGML_CUDA_AW_WAVE_INSPECT="$WAVE_INSPECT" \
    ...
    GGML_CUDA_AW_CORRIDOR_ASYNC="$CORRIDOR_ASYNC" \

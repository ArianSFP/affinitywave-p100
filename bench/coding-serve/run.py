...
DSPARK_MODEL = Path('/home/arian/models/qwen3.6-35b-a3b/Qwen3.6-35B-A3B-DSPARK-P100.gguf')
...
def main():
    ...
    parser.add_argument('--speculative', choices=['none', 'dspark', 'mtp', 'ngram-mtp'], default='none',
                        help='speculative decoder; DSpark uses the compatible Qwen3.6 draft model')
    parser.add_argument('--draft-model', type=Path, default=DSPARK_MODEL,
                        help='DSpark draft GGUF path')
    parser.add_argument('--draft-n-max', type=int, default=2,
                        help='maximum speculative draft tokens')
    parser.add_argument('--draft-p-min', type=float, default=0.0,
                        help='minimum DSpark confidence for draft pruning')
    parser.add_argument('--draft-ngl', default='all',
                        help='draft model GPU layers (default: all)')
    parser.add_argument('--draft-device', default='CUDA0',
                        help='draft model devices (default: CUDA0)')
    ...
    if args.speculative != 'none' and args.mode not in ['server', 'serve']:
        parser.error('speculative decoding is only supported for server modes')
    ...
    if args.speculative == 'dspark' and not args.draft_model.is_file():
        parser.error(f'DSpark draft model does not exist: {args.draft_model}')
    ...
    if args.mode == 'bench':
        ...
        if args.speculative != 'none':
            spec_type = {
                'dspark': 'draft-dspark',
                'mtp': 'draft-mtp',
                'ngram-mtp': 'ngram-mod,draft-mtp',
            }[args.speculative]
            cmd += ['--spec-type', spec_type, '--spec-draft-n-max', str(args.draft_n_max)]
            if args.speculative == 'dspark':
                cmd += ['--spec-draft-model', str(args.draft_model),
                        '--spec-draft-p-min', str(args.draft_p_min),
                        '--spec-draft-ngl', args.draft_ngl,
                        '--spec-draft-device', args.draft_device]

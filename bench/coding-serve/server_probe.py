...
def _probe_base(port, host):
    # Wildcard binds are reached through loopback; explicit LAN binds must use
    # the same address as the server.
    probe_host = '127.0.0.1' if host in ('0.0.0.0', '::', '') else host
    return f'http://{probe_host}:{port}'


def prewarm_server(port, sizes, ctx, stop_event=None, host='127.0.0.1'):
    base = _probe_base(port, host)
